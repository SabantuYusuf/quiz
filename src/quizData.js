export const quizData = [
    {
        id: 0,
        question: "What was Tandem previous name?",
        options: ["Tandem", "Burger Shack", "Devmynd", "Extraordinary Humans"],
        correct: "Devmynd"
    },
    {
        id: 1,
        question: "In Shakespeare's play Julius Caesar, Caesar's last words were...",
        options: ["Iacta alea est!", "Vidi, vini, vici", "Aegri somnia vana", "Et tu, Brute?"],
        correct: "Et tu, Brute?"
    },
    {
        id: 2,
        question: "A group of tigers are referred to as:",
        options: ["Ambush", "Chowder", "Pride", "Destruction"],
        correct: "Ambush"
    },
    // {
    //     id: 3,
    //     question: "What is the top speed an average cat can travel?",
    //     options: ["42 mph", "13 mph", "31 mph", "9 mph"],
    //     correct: "31 mph"
    // },
    // {
    //     id: 4,
    //     question: "A cat can jump to _____ times its own height:",
    //     options: ["3", "9", "7", "5"],
    //     correct: "5"
    // },
    // {
    //     id: 5,
    //     question: "What is the only letter that doesn't appear in a US state name?",
    //     options: ["M", "Q", "Z", "X"],
    //     correct: "Q"
    // },
    // {
    //     id:6,
    //     question: "What is the name for a cow-bison hybrid?",
    //     options: ["Cowson", "Bicow", "Beefalo", "Mooson"],
    //     correct: "Beefalo"
    // },
    // {
    //     id: 7,
    //     question: "What is the largest freshwater lake in the world?",
    //     options: ["Lake Baikal", "Lake Superior", "Lake Michigan", "Lake Victoria"],
    //     correct: "Lake Superior"
    // },

    // {
    //     id: 8,
    //     question: "In a website address bar, what does WWW stand for?",
    //     options: ["World Wide Web", "Wild Wild West", "War World Web"],
    //     answer: "World Wide Web"
    // },
    // {
    //     id: 9,
    //     question: "In a game of bingo, what number is represented by the name two little ducks?",
    //     options: ["20", "55", "77", "22"],
    //     answer: "22"
    // },
    // {
    //     id: 10,
    //     question: "According to Greek mythology, who was the first woman on Earth?",
    //     options: ["Lilith", "Eve", "Pandora", "Hera"],
    //     answer: "Pandora"
    // },
    // {
    //     id: 11,
    //     question: "In which European city would you find Orly airport?",
    //     options: ["London", "Paris", "Belgium", "Munich"],
    //     answer: "Paris"
    // },
    // {
    //     id: 12,
    //     question: "Where would you find the Sea of Tranquility?",
    //     options: ["California", "Siberia", "China", "The Moon"],
    //     answer: "The Moon"
    // },
    // {
    //     id: 13,
    //     question: "Which artist painted 'Girl with a Pearl Earrin'?",
    //     options: ["Van Gogh", "Vermeer", "Picasso", "Da Vinci"],
    //     answer: "Vermeer"
    // },
    // {
    //     id: 14,
    //     options: "What is the official name for the 'hashtag' symbol?",
    //     options: ["Octothorpe", "Number sign", "Hash Sign", "Pound"],
    //     answer: "Octothorpe"
    // },
    // {
    //     id: 15,
    //     stion: "Not American at all, where is apple pie from?",
    //     options: ["Japan", "Ethiopia", "England", "Canada"],
    //     answer: "England"
    // },
    // {
    //     id: 16,
    //     estion: "What is the national animal of Scotland?",
    //     options: ["Bear", "Unicorn", "Rabbit", "Seal"],
    //     answer: "Unicorn"
    // },
    // {
    //     id: 17,
    //     uestion: "Where in the world is the only place where Canada is *due south*",
    //     options: ["Alaska", "Russia", "Washington", "Detroit"],
    //     answer: "Detroit"
    // },
    // {
    //     id: 18,
    //     question: "Approximately how many grapes go into a bottle of wine?",
    //     options: ["500", "200", "1000", "700"],
    //     answer: "700"
    // },
    // {
    //     id: 19,
    //     question: "How much does a US One Dollar Bill cost to make?",
    //     options: ["$0.05", "$0.25", "$1", "$5"],
    //     answer: "$0.05"
    // },
    // {
    //     id: 20,
    //     question: "The Vatican bank has the only ATM in the world that allows users to do what?",
    //     options: [
    //         "Receive withdrawls in rosary beads",
    //         "Vote for the Pope",
    //         "Perform transactions in Latin",
    //         "Purchase indulgences"
    //     ],
    //     answer: "Perform transactions in Latin"
    // }
]
